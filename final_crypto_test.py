...
"""
تست ساده function جدید فرمت پیام قیمت ارز
"""

def format_crypto_message_fixed(data):
...
def test_message():
    ...
    print("🧪 تست نهایی پیام قیمت ارزها")
    print("=" * 50)
    ...
    # کاراکترهای markdown قدیمی
    if ':,.0f' in result or '+.' in result or '*' in result:
        problems.append("❌ کاراکترهای Markdown قدیمی موجود است")
    
    # کاراکترهای ممنوع
    forbidden = ['[', ']', '{', '}', '<', '>', '|', '\\', '^', '`']
    for char in forbidden:
        if char in result:
            problems.append(f"🚫 کاراکتر ممنوع '{char}' موجود است")
    ...
    if problems:
        ...
        print("\n✅ هیچ مشکل کاراکتری یافت نشد!")
        print("🚀 پیام آماده ارسال به تلگرام است!")
